...
# 자물쇠에는 홈이 파여 있고 열쇠 또한 홈과 돌기 부분이 있습니다. 열쇠는 회전과 이동이 가능하며 열쇠의 돌기 부분을
# 자물쇠의 홈 부분에 딱 맞게 채우면 자물쇠가 열리게 되는 구조입니다. 자물쇠 영역을 벗어난 부분에 있는 열쇠의 홈과 돌기는
# 자물쇠를 여는 데 영향을 주지 않지만, 자물쇠 영역 내에서는 열쇠의 돌기 부분과 자물쇠의 홈 부분이 정확히 일치해야 하며
...
def rotated_matrix_90_degree(matrix):
    rows = len(matrix)
    cols = len(matrix[0])
...
def solution(key, lock):
    len_l = len(lock)
    len_k = len(key)
    new_lock = [[0] * (len_l * 3) for _ in range(len_l * 3)]
